...
func GenerateHeatTemplate(vmsData []VMData, stackName string) (string, map[string]interface{}) {
	...
	fmt.Fprintf(&template, "description: 'Migrated VMware workloads managed by os-migrate (Stack - %s)'\n\n", stackName)
	...
	for _, vm := range vmsData {
		...
		fmt.Fprintf(&template, "    description: Boot volume ID for %s\n", vm.Name)
		...
		for i, volID := range vm.DataVolumeIDs {
			...
			fmt.Fprintf(&template, "  %s:\n", dataParamName)
			template.WriteString("    type: string\n")
			fmt.Fprintf(&template, "    description: Data volume %d ID for %s\n", i, vm.Name)
	...
	for _, vm := range vmsData {
		...
		for i := range vm.DataVolumeIDs {
			fmt.Fprintf(&template, "        - volume_id: { get_resource: %s_data_volume_%d }\n", sanitized, i)
			fmt.Fprintf(&template, "          boot_index: %d\n", i+1)
			template.WriteString("          delete_on_termination: false\n")
		}

		template.WriteString("      networks:\n")
		fmt.Fprintf(&template, "        - port: { get_resource: %s_port }\n\n", sanitized)
	}
	...
	for _, vm := range vmsData {
		sanitized := sanitizeName(vm.Name)
		fmt.Fprintf(&template, "  %s_instance_id:\n", sanitized)
		fmt.Fprintf(&template, "    description: Instance ID for %s\n", vm.Name)
		fmt.Fprintf(&template, "    value: { get_resource: %s_instance }\n", sanitized)
		fmt.Fprintf(&template, "  %s_port_id:\n", sanitized)
		fmt.Fprintf(&template, "    description: Port ID for %s\n", vm.Name)
		fmt.Fprintf(&template, "    value: { get_resource: %s_port }\n", sanitized)
	}
...
func GenerateWrapHeatTemplate(vmsData []VMData, stackName string) (string, map[string]interface{}, error) {
	...
	template.WriteString("heat_template_version: wallaby\n")
	fmt.Fprintf(&template, "description: 'Existing migrated workloads wrapped by os-migrate (Stack - %s)'\n\n", stackName)
	...
	for _, vm := range vmsData {
		...
		fmt.Fprintf(&template, "  %s:\n", instanceParam)
		template.WriteString("    type: string\n")
		fmt.Fprintf(&template, "    description: Existing Nova instance ID for %s\n", vm.Name)
		...
		for i, portID := range vm.PortIDs {
			...
			fmt.Fprintf(&template, "  %s:\n", portParam)
			template.WriteString("    type: string\n")
			fmt.Fprintf(&template, "    description: Existing Neutron port %d ID for %s\n", i, vm.Name)
	...
	for _, vm := range vmsData {
		...
		for i := range vm.PortIDs {
			fmt.Fprintf(&template, "  %s_port_%d:\n", sanitized, i)
			template.WriteString("    type: OS::Neutron::Port\n")
			fmt.Fprintf(&template, "    external_id: { get_param: %s_port_%d_id }\n\n", sanitized, i)
		}

		fmt.Fprintf(&template, "  %s_instance:\n", sanitized)
		template.WriteString("    type: OS::Nova::Server\n")
		fmt.Fprintf(&template, "    external_id: { get_param: %s_instance_id }\n\n", sanitized)
	}
	...
	for _, vm := range vmsData {
		...
		fmt.Fprintf(&template, "    value: { get_resource: %s_instance }\n", sanitized)
		for i := range vm.PortIDs {
			fmt.Fprintf(&template, "  %s_port_%d_id:\n", sanitized, i)
			fmt.Fprintf(&template, "    description: Port %d ID for %s\n", i, vm.Name)
			fmt.Fprintf(&template, "    value: { get_resource: %s_port_%d }\n", sanitized, i)
		}
	}
